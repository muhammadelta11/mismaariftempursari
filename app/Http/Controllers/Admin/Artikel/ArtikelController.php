    ...
    public function index(Request $request)
    {
        if (request()->ajax()) {
            if (!auth_has_role(1)){
                ...
                $model = Artikel::select (['id', 'nama', 'slug', 'excerpt', 'status', 'created_at', 'date','user_id'])->where('user_id','=',auth()->user()->id)//->join('users', "$tbl.id", '=', "users.id")
                //->where($request->id, "=", "$tbl.id")
                ->selectRaw('IF(status = 1, "Dipublish", "Disimpan") as status_str');
            }else{
  
                $model = Artikel::select (['id', 'nama', 'slug', 'excerpt', 'status', 'created_at', 'date','user_id'])   //->join('users', "$tbl.id", '=', "users.id")
                ->selectRaw('IF(status = 1, "Dipublish", "Disimpan") as status_str');
            }
            // filter
