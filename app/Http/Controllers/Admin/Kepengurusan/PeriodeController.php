    ...
    public function set_pengurus_role(Request $request)
{
    ...
    try {
        ...
        // Fetching the list of members with their associated roles and users
        $lists = Anggota::join($t_jabatan, "$t_jabatan.id", "=", "$t_anggota.jabatan_id")
            ->where("$t_jabatan.periode_id", $periode)
            ->get();

        // Prepare roles array for syncing
        $rolesToSync = [];

        foreach ($lists as $v) {
            $user = $v->anggota->user;

            // Skip the super admin role
            if ($user->hasRole(config('app.super_admin_role'))) {
                continue;
            }

            // Get the current role or use the requested one
            $role_by_jabatan = $v->role_id ? Role::find($v->role_id)->name : null;
        ...
        return response()->json([
            ...
            'roles' => array_unique($rolesToSync), // Optionally return the updated roles
        ]);
